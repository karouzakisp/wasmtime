...
use crate::ir::{Block, Function, Inst, Opcode, Value, ValueDef};
...
#[derive(Clone, Copy, Debug, PartialEq, Eq)]
struct LoadUserMetadata {
    schedule_priority: i32,
    load_inst: Inst,
    user_inst: Inst, // The Instruction that uses the load result.
}

impl LoadUserMetadata {
    fn decrement_priority(&mut self) {
        self.schedule_priority -= 1;
    }
}

impl Ord for LoadUserMetadata {
    #[inline]
    fn cmp(&self, other: &Self) -> std::cmp::Ordering {
        self.schedule_priority.cmp(&other.schedule_priority)
    }
}

impl PartialOrd for LoadUserMetadata {
    fn partial_cmp(&self, other: &Self) -> Option<core::cmp::Ordering> {
        Some(self.cmp(other))
    }
}
...
impl<'a> Elaborator<'a> {
    ...
    fn is_load(&self, inst: Inst) -> bool {
        match self.func.dfg.insts[inst].opcode() {
            Opcode::Load
            | Opcode::StackLoad
    ...
    fn schedule_insts(&mut self, block: Block) {
        ...
        while let Some((original_inst, _)) = self.ready_queue.pop() {
            ...
            for value in &unique_original_values {
                // Remove the instruction from the argument's value users.
                let BestEntry(_, best_value) = self.value_to_best_value[*value];
                trace!(
                ...
                // and update its node in the ready queue.
                //
                // If the value was generated by a slow instruction, skip incrementing the LUC.
                if self.value_users[best_value].len() == 1 {
                    if let Some(value_def) = self.func.dfg.value_def(best_value).inst() {
                        if match self.func.dfg.insts[value_def].opcode() {
                            Opcode::Load
                            | Opcode::StackLoad
                            | Opcode::DynamicStackLoad
                            | Opcode::AtomicLoad
            ...
            if !scheduled_was_load {
                ...
                scheduled_load_users.retain(|metadata| {
                    if metadata.schedule_priority == 0 || self.ready_queue.len() == 0 {
                        ...
                        false // Remove this element from `scheduled_load_users`
                    } else {
            ...
            for result in self.func.dfg.inst_results(original_inst).iter().cloned() {
                ...
                for user_inst in self.value_users[result].iter().cloned() {
                    ...
                    // Deprioritize users of load instructions.
                    if scheduled_was_load {
